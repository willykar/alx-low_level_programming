 ...
 *
 * Return: (NULL) if the key cannot be matched or
 * the value associated with key in ht.
 */
char *shash_table_get(const shash_table_t *ht, const char *key)
{
	shash_node_t *node;
	unsigned long int inex;

	if (ht == NULL || key == NULL || *key == '\0')
		return (NULL);

	inex = key_index((const unsigned char *)key, ht->size);
	if (inex >= ht->size)
		return (NULL);
 ...
 * @ht: A pointer to the sorted hash table that will be printed
 */
void shash_table_print_rev(const shash_table_t *ht)
{
	shash_node_t *node;
